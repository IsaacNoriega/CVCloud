  ...
  return (
    <div className="w-full h-full bg-white p-16 space-y-10">
      ...
      <div className="text-center space-y-3 pb-8 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500">{data.email} {data.phone && `• ${data.phone}`}</p>
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            {data.education.map((edu) => (
              <div key={edu.id} className="text-center">
                ...
                <p className="text-sm text-gray-500">{edu.institution} {edu.period && `• ${edu.period}`}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {data.languages.map((lang) => (
              <span key={lang.id} className="text-sm text-gray-700">
                {lang.language || 'Idioma'} <span className="text-gray-400">({lang.level})</span>
              </span>
            ))}
          </div>
        </div>
